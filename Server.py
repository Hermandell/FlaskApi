...
class MyServer:
    def __init__(self):
        ...
        #maestros
        self.LoginUrlMaestro = ('https://itnleon.mindbox.app/login/autoriza-administrativo')
        self.ProfileMaestro = ('https://itnleon.mindbox.app/docentes/datos')
        self.maestroCalificaciones=('https://itnleon.mindbox.app/docente/calificar/parciales/20231/18381')
    ...
    def loginMaestros(self):
        ...
        try:
            ...
            if (not errores):
                ...
                s = self.sesion
#-+---------------------aqui checar porque el api se envia aun cuando da error de credencial----------------
                html=s.post(self.LoginUrlMaestro, cookies=r.cookies, data=credentials).text
                #print(html)
                # si no tiene un dropdown menu entonces significa que no se pudo loguear bien
                htmlLog=BeautifulSoup(html, 'html.parser').find('ul', {'class': 'dropdown-menu'})
                if htmlLog is None:
